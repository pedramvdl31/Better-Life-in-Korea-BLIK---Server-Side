...
public function postSlidersEdit()
{
       ...
       if (isset($deleted_image_names) && !empty($deleted_image_names)) {
        // Job::dump($deleted_image_names);
           foreach ($deleted_image_names as $dinkey => $dinvalue) {
            $path = public_path("assets/images/sliders/".$title."/".$dinvalue);
            if (file_exists($path)) {
                unlink($path);
            } 
    ...
    if (!file_exists('assets/images/sliders/'.$title_lowered)) {
        mkdir('assets/images/sliders/'.$title_lowered, 0777, true);
    } 
    if (isset($new_image_name)) {
       foreach ($new_image_name as $neinkey => $neinvalue) {
        $oldpath = public_path("assets/images/sliders/temp/".$neinvalue);
        $newpath = public_path("assets/images/sliders/".$title."/".$neinvalue);
