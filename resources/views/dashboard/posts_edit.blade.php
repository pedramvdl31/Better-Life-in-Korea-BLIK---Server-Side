...
<div class="panel panel-default">
  <div class="panel-body">
   ...
   <div class="form-group" id="cat-wrap">
	...
	<div class="form-group 2t-wrap" id="title-wrap">
	...
	<div class="form-group 2t-wrap" id="des-wrap">
		...
		@if(isset($ps->decoded_files))
			@foreach($ps->decoded_files as $pfk => $pfv)
				@foreach($pfv as $pk => $pv)
					<input class="posted-files" old-name="{{$pv['name']}}" name="posted_files[{{$pfk}}][{{$pk}}][name]" type="hidden" value="{{$pv['name']}}"/>
				@endforeach
			@endforeach
		@endif
	</div>
	    ...
	    <div class="row thumb-row" style="margin: 0;">
		    @if(isset($ps->decoded_files))
				@foreach($ps->decoded_files as $pfk => $pfv)
					@foreach($pfv as $pk => $pv)
						@if($pk=="image")
					      <div class="col-xs-4 col-md-2 thumb-wrap">
